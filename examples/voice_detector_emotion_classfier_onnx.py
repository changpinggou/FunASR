...
class VoiceDetectorEmotion(VoiceDetector):
    def __init__(self, model_path="/data/applechang/FunASR/examples/emotion2vec_plus_large_onnx/emotion2vec"):
        """
        初始化ONNX情感检测器
        ...
        self.ort_classfier_session = ort.InferenceSession("/data/applechang/emotion2vec_applechang/emotion2vec/fast_train_output/classifier_head.onnx", session_options)
    ...
    def detect(self, audio_source, source_type="buffer", user_prompt=None)->dict:
        ...
        try:
            ...
            feats = outputs[0] # 形状 (1, T, 1024)
            # 第二步：平均池化 (Mean Pooling)
            pooled_feats = np.mean(feats, axis=1) # 形状 (1, 1024)

            # 第三步：运行刚导出的分类头
            head_outputs = self.ort_classfier_session.run(None, {'input': pooled_feats})
            logits = head_outputs[0] # 形状 (1, 8)
